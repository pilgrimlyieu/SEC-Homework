...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for bee in self.place.bees[:]:
            bee.reduce_health(amount + (self.damage if amount >= self.health else 0))
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.slowed_time > 0 and gamestate.time % 2 == 1:
            self.slowed_time -= 1
        else:
            self.slowed_time  = max(self.slowed_time - 1, 0)
            if self.blocked():
                self.sting(self.place.ant)
            elif self.health > 0 and destination is not None:
                self.move_to(destination)
                self.scared_time = max(self.scared_time - 1, 0)
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        damage = self.damage - 0.25 * distance - self.insects_shot / 16
        return max(0, damage)
        # END Problem Optional 2
