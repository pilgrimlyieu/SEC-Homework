...
def roll_dice(current_score, num_rolls, dice=six_sided):
    ...
    for _ in range(num_rolls):
        score = dice()
        parity_score += 1 - (current_score - score) % 2
        if score == 1:
            dices_score = 1
            pigout_status = True
        elif not pigout_status:
            dices_score += score
    return dices_score + parity_score
    # END PROBLEM 1
...
def picky_piggy(score):
    ...
    """
    # BEGIN PROBLEM 2
    return 1 if score == 0 else (10 ** ((score % 6) or 6) // 21) % 10
    # END PROBLEM 2
...
def take_turn(num_rolls, current_score, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    # BEGIN PROBLEM 3
    return picky_piggy(opponent_score) if num_rolls == 0 else roll_dice(current_score, num_rolls, dice)
    # END PROBLEM 3
...
def swine_swap(player_score, opponent_score):
    ...
    return int(str(excitement)[0]) == excitement % 10
    # END PROBLEM 4
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        points = 0
        if (who == 0 and score0 - last_score > running_high) or \
           (who == 1 and score1 - last_score > running_high):
            highest = who
        else:
            highest = None
        if highest is not None:
            points = [score0, score1][who] - last_score
            print(f"{points} point(s)! That's a record gain for Player {who}!")
        return announce_highest(who, [score0, score1][who], points if (highest is not None) else running_high)
    return say
    # END PROBLEM 7
...
def make_averaged(original_function, trials_count=1000):
    ...
    def average(*args):
        ...
        for _ in range(trials_count):
            sum_count += original_function(*args)
        return sum_count / trials_count
    return average
    # END PROBLEM 8
...
def max_scoring_num_rolls(current_score=0, dice=six_sided, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 9
    average_scores = [0 for _ in range(10)]
    for i in range(1, 11):
        average_scores[i - 1] = make_averaged(roll_dice, trials_count)(current_score, i, dice)
...
def run_experiments():
    ...
    six_sided_max = max_scoring_num_rolls(dice=six_sided)
    print('Max scoring num rolls for six-sided dice:', six_sided_max)
    from itertools import combinations
    for i, j in combinations(range(0, 11), 2):
        print(f'always_roll({i}) v.s. alwats_roll({j}) win rate: {100 * average_win_rate(always_roll(i), always_roll(j)):2f}%')
